...
const Governing = () => {
  return (
    <section className=" pt-5 pb-5 bg-amber-50">
        ...
        <h1 className=" bg-amber-300 p-5 font-bold w-11/12 md:w-10/12">Governing Body Members</h1>
        </div>
        <div className=' flex justify-center'>
            <div className=' w-11/12 md:w-10/12 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-5 mb-5'>
                {
                    governing.map((item)=>
                      <div key={item.id} className=' bg-white shadow-md hover:shadow-md rounded-md transition duration-150 hover:scale-105'>
                        <div className=' flex justify-center'> <Image src={item.Photo} alt='img' className=' h-40  w-full rounded-md shadow-sm p-1 bg-white'/></div>
                         <div className='flex-col justify-center p-4'>
                            <h1 className=' font-semibold text-center text-sm'>{item.Names}</h1>
                            <h1 className=' text-sm text-gray-700 text-center'>{item.Designation}</h1>
                         </div>
                      </div>
                    )
                }
